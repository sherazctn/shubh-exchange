...
const Fancy = ({ market, marketType, oddsPrice, webColor, eventId, tabs, setTabs, eventName, pendingBets, oneTouchEnable, trigger, fancyRate }: any) => {
  ...
  const handleNewBetClicked = (e: any, item: any, side: any, selectionName: string, marketName: string, marketId: string, gameName: string, gameId: string) => {
    ...
    const obj = {
      afterLoss: wallet - 10,
      afterWin: wallet + profit,
      exposure: marketType === "m1" ? (side === "Back" ? -10 : -Number(((parseFloat(item?.odds) / 100) * 10).toFixed(2))) : (side === "Back" ? -10 : -Number(((parseFloat(item?.size) / 100) * 10).toFixed(2))), //
      profit: marketType === "m1" ? (side === "Back" ? Number(((parseFloat(item?.odds) / 100) * 10).toFixed(2)) : 10) : (side === "Back" ? Number(((parseFloat(item?.size) / 100) * 10).toFixed(2)) : 10), //
    ...
    if(marketType === "m1") updatedCalculation = marketOddsFormulation(obj, updatedPendingBets);
                  ...
                  <div className="flex flex-wrap sm:gap-[11px] justify-end sm:justify-center items-center relative">
                    {item?.odds?.map((i: any) => (
                      <div
                        ...
                        onClick={(e) => handleNewBetClicked(e, i, "Back", item?.nat, market?.mname, market?.mid, eventName, item?.sid)}
                        onMouseDown={(e) => handleStart(e, item, '1')}
                        onTouchStart={(e) => handleStart(e, item, '1')}
                      >
                        <p className="font-[800] text-center text-[13px] sm:text-[15px]">
